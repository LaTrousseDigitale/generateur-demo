...
export const DemoFeaturesDisplay = ({ config }: DemoFeaturesDisplayProps) => {
  ...
  if (config.ecommerceNeeds?.length > 0) {
    config.ecommerceNeeds.forEach(feature => {
      allFeatures.push({ name: feature, category: "E-commerce" });
    });
  }
  ...
  if (config.autoCompatibility?.length > 0) {
    config.autoCompatibility.forEach(feature => {
      allFeatures.push({ name: feature, category: "Automobile - Recherche" });
    });
  }

  if (config.autoSearchFeatures?.length > 0) {
    config.autoSearchFeatures.forEach(feature => {
      allFeatures.push({ name: feature, category: "Automobile - Fonctionnalités" });
    });
  }
  ...
  if (config.restaurantFeatures?.length > 0) {
    config.restaurantFeatures.forEach(feature => {
      allFeatures.push({ name: feature, category: "Restauration" });
    });
  }
  ...
  if (config.retailFeatures?.length > 0) {
    config.retailFeatures.forEach(feature => {
      allFeatures.push({ name: feature, category: "Commerce de détail" });
    });
  }
          ...
          {Object.entries(featuresByCategory).map(([category, features]) => (
            <Card key={category} className="p-6">
              <h3 className="font-bold text-xl mb-4 flex items-center gap-2">
                <Settings className="w-5 h-5" style={{ color: config.primaryColor }} />
                {category}
              </h3>
                ...
                {features.map((feature, index) => {
                  ...
                  return (
                    <div key={index} className="flex items-start gap-3 p-3 rounded-lg hover:bg-muted/50 transition-colors">
                      <div className="flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center" style={{ backgroundColor: config.primaryColor + "15" }}>
